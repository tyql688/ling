...
2. Inventory the SDK pins defined by [update-pi-dependency](../update-pi-dependency/SKILL.md) and all bundled extensions, even when updating only one. The extensions are exact Host dependencies: `@juicesharp/rpiv-todo`, `@gotgenes/pi-permission-system`, `@earendil-works/pi-voice` and `pi-mcp-adapter`.
...
2. Build the affected Host and Web. Load the published SDK root in plain Node and load the actual extension entries through Pi. Inspect a fresh isolated staged Host with its standalone Node when dependency or package contents change, including declared entries, transitive dependencies, WASM assets, native voice libraries and MCP optional native keyring binaries. Preserve any running application's `.stage`.

...
macOS and Windows release jobs require the existing reusable signing certificate. Preserve that identity; do not generate or rotate signing material as a routine fix. The current certificate does not establish Apple notarization or public Windows trust. Linux artifacts are unsigned by this workflow.

macOS CI checks signatures and native architectures and executes bundled Node. Windows CI inspects Authenticode. Neither runs the Electron UI or proves installation and auto-update behavior.

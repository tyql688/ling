...
packages/host/src/
  ...
  domains/
    ...
    pi-adapters/           bundled Pi packages: todo, permission activation, voice input and MCP services
...
`domains/pi-adapters/` adapts four bundled Pi packages. Project catalogs retain the selected bundled entries as metadata; only session graphs execute their factories. Independently installed Pi extensions still load in project catalogs because they may contribute providers and resources. Native voice operations resolve the selected installed or bundled entry from that same catalog, preserving package precedence. `adapter-plan.ts` resolves their entry files once and answers the worker's `adapters.read` request per project: the feature switches, enabled bundled Todo, Voice and MCP entries, permission package entry and effective permission activation for that project. `todo/` projects the latest `todo` tool result and can start a reconciliation run; `permission-system/` owns activation writes, the reload they trigger and the rule file entry points.
...
`mcp/` owns native configuration management for bundled `pi-mcp-adapter` 2.37.0. Core reads the adapter's public configuration API and retains its merge, imports, lazy connections, proxy/direct tools, OAuth and approval behavior. Settings edits Pi global or project `mcp.json` and the optional shared files without a parallel Ling format. Bounded JSONC reads reject malformed documents; revision-checked, locked atomic updates preserve comments, unrelated fields, symlinks and file permissions. Only the selected service is changed; a project disable writes `{ "disabled": true }` without copying global credentials. Mutations use the shared resource reload coordinator, including after partial failure. Unchanged submissions preserve the file and skip reload; changed configuration targets projects that actually loaded an MCP adapter, including independently installed copies with the Ling switch off. In-flight project lifecycle changes retain conservative reconciliation. MCP settings receive a dedicated change event even when no runtime needs reloading. Optional imports and package defaults remain adapter-owned. `mcp-settings.ts` owns shared mutation admission for Settings and the always-registered `ling_mcp` conversation tool. The tool accepts only its managed session project, reports credential-free inventories, and merges explicit patches without reconstructing saved secrets. Configuration edits never enable MCP; explicit feature changes use the same revision-checked switch/reload path as Settings. Busy-session reloads remain pending until the turn ends.

MCP extensions are excluded from untrusted projects because their own loader reads project configuration independently of Pi's resource inventory. A separately installed npm or official Git copy takes precedence. The built-in switch does not disable that user's Pi package. For verified 2.37.0 copies, Ling adapts `/mcp`, `/mcp status`, `/mcp setup` and `/mcp edit` to Settings and subscribes to the public versioned status event. A session-owned listener is attached before adapter startup and removed at shutdown; generation-bound UI capabilities reject late updates. Cached tool metadata is not shown as a live connection. Unknown versions keep original commands. Reconnect and authentication use a runtime-bound MCP request and the existing approval UI. Only the verified extension's registered command can execute; a missing command never becomes a model prompt.

In the worker, `core/pi-sdk/extensions/pi-adapters.ts` builds the project's extension list: every enabled user extension, plus a bundled package only when the user has not installed the same package through Pi, minus every copy of the permission system when the project runs with full access. Bundled entries are decorated with `ling:todo`, `ling:voice`, `ling:mcp` and `ling:permission-system` source identities. `pi-tool-origin.ts` records each tool call's owning extension on the transcript branch before execution; `pi-resource-identity.ts` reads the loaded inventory's identities and versions. Features project only results whose recorded origin they verified and otherwise keep Pi's rendering. `packages/builtin-extensions/src/plugins/companion-tools/` registers only enabled companion tools with Pi and forwards each call to the Host.

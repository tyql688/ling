...
Centered dialogs divide their viewport width and height limits by the browser shell's CSS zoom factor. Native page zoom keeps that factor at one because Electron already adjusts the CSS viewport. Dialog form bodies scroll within these limits while their actions remain reachable.
...
Settings → Plugins presents the seven Ling features as shared toggle rows before Pi package management. Each row links to its owning documentation; adapter settings retain the same link, including while the feature is off. These switches control runtime resources, not just visual elements. Keep the saved choice visible during reload and report deferred or failed reloads. A disabled feature retains readable history and cleanup actions, with an inline notice linking to Plugins; prevent new work at both the UI and Host. Permission controls show that the master switch is off rather than claiming a saved approval mode is active. Scheduled tasks show suspended triggers while the feature is off.

MCP services are off by default. Settings provides a global/project selector, the selected file's services, effective project configuration and live status for the current conversation. Adding a service supports executable/argument and HTTP/SSE forms plus upstream JSON options. New entries request tool approval and connect on demand; existing advanced options are preserved. Tool approval has an explicit selector for asking, no additional MCP confirmation, inheritance and tool patterns; the JSON draft remains its single source. Field errors are localized, focus the relevant input and identify it to assistive technology. Keep dialog actions visible while the form body scrolls. Show configuration paths, project overrides and read/write failures. A global service can be disabled only in the current project without copying credentials. Restoring the inherited enable state removes only the project disabled property, drops an otherwise empty override and keeps other project options. File revision conflicts keep the draft open and offer reading the latest configuration for explicit review before another save; never silently replace an external edit. Read failures disable writes until a successful refresh. Removing an override explains that a lower-priority entry may become effective. Keep configuration management available when the bundled runtime is off, and explain that a separately installed Pi adapter retains its own enable state. The `ling_mcp` conversation tool follows the same rule: configuration, a server disabled override and the global feature switch are separate actions. Keep the `ling-and-pi` skill independent of MCP activation and describe pending reloads separately from verified connections. A compact Composer indicator links to settings when the adapter exposes live status. Unavailable connection actions explain whether the adapter is off, the conversation is busy or a service name is unsupported. `/mcp` opens the same page; reconnect and sign-in return to the owning conversation for prompts and approvals.
